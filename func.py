...
def add_poly_features(data, column_names):
    ...
    train_null = pd.isnull(features)
    train_null = features[train_null == True]
    print(train_null)
 
    #缺失值处理，以下两种方式均可
    #删除包含缺失值的行
    features.dropna(inplace=True) # 43755, 9 => 43716, 9
    # #缺失值填充
    # features.fillna('100')
    ...
    for col in poly_features.columns: # 54
        if col in rest_features.columns.tolist():
            continue
        rest_features.insert(1, col, poly_features[col]) # 77
...
def numerical_4_feature(feature_col, year_col, month_col, day_col, irradiance_col, method = None, time_period = None):
    ...
    if time_period == 'daytime':
        ...
        # index
            ...
            b = feature_col[(year_col == s_y) & (month_col == s_m) & (day_col == s_d) & (irradiance_col != 0)]
...
def lgb_train_actual_irradiance(X_train, y_train, X_validation, y_validation, test_features, params, column_name, experiment_time, train_list):
    ...
    lgb.plot_importance(gbm, max_num_features = 100, figsize = (12, 16))
    
    title = "FeatureImportance actual irradiance " + train_list
    plt.title(title)
    
    save_path = 'figure/' + experiment_time
...
def lgb_train_actual_power(X_train, y_train, X_validation, y_validation, test_features, params, column_name, experiment_time, train_list):
    ...
    matplotlib.rc("font",family='AR PL UMing CN',weight="bold")
    ...
    title = "FeatureImportance actual power " + train_list
    plt.title(title)
    name = title + '_' + train_list + '.pdf'
    save_path_name = save_path + '/' + name
    plt.savefig(save_path_name)
    
    plt.clf()
    
    # lgb.plot_metric(evals_result, metric="mse")
    lgb.plot_tree(gbm, tree_index=3, figsize=(20, 8), show_info=['split_gain'])
    title = "Plotting metrics recorded during training " + train_list
    plt.title(title)
    name = title + '_' + train_list + '.pdf'
    save_path_name = save_path + '/' + name
    plt.savefig(save_path_name)
