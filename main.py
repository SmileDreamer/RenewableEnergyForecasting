...
if __name__ == '__main__':
    ...
    for i in range(len(train_list)):
        ...
        ### 第一次训练的特征工程
        train_features, test_features, label_1, label_final= first_feature_engineering(train_old, test_old, train_list[i], test_list[i])
        ...
        ### xgb第一次训练
        pred_label_1_xgb = xgb_train_actual_irradiance(X_train_1, y_train_1, X_validation_1, y_validation_1, test_features, xgb_params, ['实发辐照度'], experiment_time, train_list[i]) # result is a score
        
        ### 对lgb训练得到的结果（实发辐照度）作为新特征加入测试集特征
        train_features_new, test_features_new_lgb = second_feature_engineering(train_features, label_1, test_features, pred_label_1_lgb, train_old, test_old)
        
        ### 对xgb训练得到的结果（实发辐照度）作为新特征加入测试集特征
        train_features_new, test_features_new_xgb = second_feature_engineering(train_features, label_1, test_features, pred_label_1_xgb, train_old, test_old)
        ...
        ### 对结果进行特殊化处理 进一步减小误差
        pred_label_final_lgb = specialize(train_list[i], pred_label_final_lgb, train_old, test_old)
        
        ### 对结果进行特殊化处理 进一步减小误差
        pred_label_final_xgb = specialize(train_list[i], pred_label_final_xgb, train_old, test_old)
        ...
        result_single_lgb = pred_label_final_lgb
        result_single_xgb = pred_label_final_xgb
    ...
    # result_lgb = result_1_lgb # (11808, 2) 其中一个维度是id
    ...
    # result_xgb = result_1_xgb # (11808, 2)
    ...
    # print("total time used is %fmin %fs" % (last // 60,  last % 60))
    
    
    # without id:    
    result_1_lgb = pd.read_csv(path + '/result_lgb_1_' + experiment_time +'.csv') # without id
    # result_2_lgb = pd.read_csv(path + '/result_lgb_2_' + experiment_time +'.csv')
    # result_3_lgb = pd.read_csv(path + '/result_lgb_3_' + experiment_time +'.csv')
    # result_4_lgb = pd.read_csv(path + '/result_lgb_4_' + experiment_time +'.csv')
    
    # result_lgb = pd.concat([result_1_lgb, result_2_lgb, result_3_lgb, result_4_lgb], axis = 0)
    result_lgb = result_1_lgb # (11808, 1)
    result_lgb.to_csv(path + '/result_total_lgb_' + experiment_time + '.csv', index = False, sep = ',', encoding = 'UTF-8')
    
    result_1_xgb = pd.read_csv(path + '/result_xgb_1_' + experiment_time +'.csv') # without id
    # result_2_xgb = pd.read_csv(path + '/result_xgb_2_' + experiment_time +'.csv')
    # result_3_xgb = pd.read_csv(path + '/result_xgb_3_' + experiment_time +'.csv')
    # result_4_xgb = pd.read_csv(path + '/result_xgb_4_' + experiment_time +'.csv')
    
    # result_xgb = pd.concat([result_1_xgb, result_2_xgb, result_3_xgb, result_4_xgb], axis = 0)
    result_xgb = result_1_xgb # (11808, 1)
    result_xgb.to_csv(path + '/result_total_xgb_' + experiment_time + '.csv', index = False, sep = ',', encoding = 'UTF-8')
    ...
    gt_power = test_old['实际功率']
    power_predict = result_ensemble['predicition']
